...
int main (void)
{
    ...
    int t = 0;         
    int j = 0;         
    long long int operNum = 0;    
    contacts* number[300];       
    contacts* update = (contacts*)malloc(sizeof(contacts));  
    int i = 0;        
    char command[7] = "";      
    command[0] = getchar();
    while(command[0]!=EOF)
    {
        ...
        if(strcmp(command,  "ADD" )==0)
        {
            ...
            scanf("%s %s",number[i]->mail,number[i]->phone);
        ...
        else if(strcmp(command,  "UPDATE")==0)
        {
            scanf("%s",update->name);
            scanf("%lld ",&update->ID);
            scanf("%s %s",update->mail,update->phone);
            getchar();
            for(t=0;t<i;t++)
            {
                if(number[t]->ID==update->ID)
                {
                    ...
                    strcpy(number[t]->phone,update->phone);
