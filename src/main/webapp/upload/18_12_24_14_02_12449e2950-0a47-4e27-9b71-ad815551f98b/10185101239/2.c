...
int add(struct link** st1,int k)
{
    char a,m[12]={0},data[30];
    a=getchar();
    if(a=='N')
{strcpy(st1[k]->num,"NULL");
    strcpy(st1[k]->num,"NULL");
    getchar();
    getchar();
    return 1;
...
int del(struct link** stu,int k,int i){
    for(int j=i;j<k-1;j++){
...
int print(struct link** stu,int k)
{
...
void b(int p,int j,int *is,int *no,int l,int i){
    for(int x=l;x<p-1;x++)
        is[l]=is[l+1];
    for(int x=0;x<p-1;x++){
            if(is[x]>i)
