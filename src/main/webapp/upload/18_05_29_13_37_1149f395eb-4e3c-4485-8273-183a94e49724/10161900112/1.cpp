...
// bool right_banlance2(AVL_node *&sub_root);
...
Error_code AVL_tree::avl_insert(AVL_node *&sub_root, const Entry &new_data,bool &taller)
{
    ...
    {
        status=avl_insert(sub_root->left,new_data,taller);
        if(taller==true)
        {
    ...
    {
        status=avl_insert(sub_root->right,new_data,taller);
        if(taller==true)
        {
            switch(sub_root->get_balance())
            {
...
void AVL_tree::left_balance(AVL_node *&sub_root)
{
    ...
    case left_higher:
        ...
        break;
    case equal_height:
        cout<<"Warning,that's impossible."<<endl;
        break;
    case right_higher:
...
{
    AVL_node *left_tree1=sub_root->left;
    //cout<<"挂右树"<<left_tree1->right->data<<endl;
    sub_root->left=left_tree1->right;
   // cout<<"翻转root"<<left_tree1->data<<endl;
    left_tree1->right=sub_root;
    sub_root=left_tree1;
}
